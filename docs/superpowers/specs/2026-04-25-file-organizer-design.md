...
- **Reconciliation on startup.** Engine runs two additive passes. First, it checks for `operations` rows with `status='in-progress'` and reconciles filesystem state vs claimed state — fix up the catalog, or mark the op `failed`. Second, it walks each registered drive's `_FileOrganizer_quarantine/` folder and cross-references every file found against the `quarantine` table: any file present on disk but absent from the table is classified as a quarantine orphan (indicative of a crash between `renameSync` and `INSERT INTO quarantine`), logged as a structured `warn` entry, and surfaced in the reconcile result for user-initiated recovery. Drives whose mount path is unreachable (e.g., NAS disconnected) are skipped with a warning — startup is not aborted.
