...
function Probe({ map }: ProbeProps) {
  ...
  return h(
    ...
    h('select', { 'data-testid': 'select' }, h('option', null, 'x')),
    h('div', { contenteditable: 'true', 'data-testid': 'editable' }),
    h('input', { type: 'search', 'data-testid': 'search' }),
  );
}

const probe = (map: ShortcutMap) => render(h(Probe, { map }));
